...
export const getTopNStoreByMetric = async (
  metric: string,
  n: number,
  ...
  try {
    const response = await axios.get<TopStores[]>(
...
export const getStoreNames = async (selected_regions: string[]) => {
  try {
    const response = await axios.get<{ store_id: string; name: string }[]>(
      `${BASE_URL}/filters/stores?selected_regions=${selected_regions}`
    );
    return response.data;
  } catch (error) {
